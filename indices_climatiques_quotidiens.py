...
------------------------------
PRINCIPAUX ATTRIBUTS DU MODULE : 
------------------------------
    data_frame (DataFrame) Tableau de données principales avec les données récupérées à chaque jour.
    dates_saisons (DataFrame) Tableau de données avec les dates des saisons et les indices climatiques.
    saison (str) : 'Gel' ou 'Degel' selon la saison désirée.
"""
...
class IndicesClimatiques:
    ...
    def __init__(self, df_sila, repertoire):
        ...
        df_indices (DataFrame) : Tableau de données avec les dates de débuts de saisons de gel 
...
if __name__ == '__main__':
    ...
    df_sila = pd.read_csv('Station_Data/CEN_SILA/SILA_Salluit_AirTemp.csv')

    from donnees_sila import DonneesSila

    donnees = DonneesSila(repertoire)                             
    donnees.donnees_sila()        
    
    indices = IndicesClimatiques(donnees.df_sila, repertoire)
    
    # Appel aux méthodes pour mettre en forme les tableaux de données
    indices.serie_saisons('Gel')
    indices.annee_climatique('Gel')
    
    df_sila = indices.df_sila
    df_indices_clim = indices.df_indices_clim
