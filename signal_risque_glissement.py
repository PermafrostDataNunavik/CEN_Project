...
des stations (active ou inactive) et pour l'envoie du signal d'alerte au besoin 
...
# =============================================================================
        ...
        # Lecture du fichier avec le statut d'envoie du signal d'alerte
        self.fichier_statut_signal = f'{self.repertoire}Rapport_PDF/statut_courriel.txt'
        lecture_statut = open(self.fichier_statut_signal, 'r')
        self.statut_signal = lecture_statut.readline()
        lecture_statut.close()  # Fermeture du fichier pour pouvoir réécrire le nouveau statut
...
# =============================================================================
    ...
    def generer_rapports(self):
        """
        Méthode principal pour créer les deux rapports (statut de transmission et niveua de risque)
    ...
    def rapport_transmission_donnees(self):
        """
        Création du rapport avec les statuts de transmission de chaque stations. 
        """
    ...
    def statut_station(self, df, station):
        """
        Détermine la dernière date de transmission. Si la transmission de données a été interrompu 
...
# =============================================================================
    ...
    def niveau_risque_glissement(self):
        ...
        et écriture du statut d'envoie du signal d'alerte.
        """
        if self.variation_cumul > 25:
            # Le signal d'alerte n'a pas été envoyé, donc on l'envoie
            if self.statut_signal == 'Aucun signal envoye': 
                self.courriel()
            else:
                print(self.statut_signal)
        else:
            # Si le risque calculé est faible, le statut d'envoie du signal reste 'non envoyé'
            self.statut_signal = 'Aucun signal envoye'
            self.ecriture_statut_signal = open(self.fichier_statut_signal, 'w')
            self.ecriture_statut_signal.write(self.statut_signal)
            self.ecriture_statut_signal.close()
    ...
    def courriel(self):
        ...
        """
# =============================================================================
# Prodondeur de dégel de la veille disponible (self.profondeur_degel) 
...
# =============================================================================
        ...
        contents=[texte_rapports, texte_risque, web_app, signature], 
        attachments=[self.rapport_niveau_risque, self.rapport_statut_stations]) 
        
        # On modifie le statut d'envoie du signal pour 'envoyé' pour ne pas l'envoyer à chaque fois. 
        self.statut_signal = f'Signal envoye - {self.date}'
        self.ecriture_statut_signal = open(self.fichier_statut_signal, 'w')
        self.ecriture_statut_signal.write(self.statut_signal)
        self.ecriture_statut_signal.close()
        
        print(f'Signal d\'alerte et rapports envoyés à {receiver}.')
...
if __name__ == '__main__':
    ...
    destinataire = 'sarah.gauthier.1@ulaval.ca'
...
# =============================================================================
    ...
    # destinataire = 'samuel.boudreault@arc.ulaval.ca'    
    # destinataire = 'jcdrolet5@hotmail.com'
    # destinataire = 'michel.allard.3@ulaval.ca'
